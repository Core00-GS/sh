...
dd_xitong() {
		...
		dd_xitong_2() {
		  echo -e "重裝後初始使用者名稱:${gl_huang}Administrator${gl_bai}  初始密码: ${gl_huang}Teddysun.com${gl_bai}  初始端口: ${gl_huang}3389${gl_bai}"
		  echo -e "按任意鍵繼續..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  if [ -n "$current_mode" ]; then
		  ...
		  echo -e "当前模式: ${gl_hui}未最佳化${gl_bai}"
	  fi
  ...
  while true; do
	...
	32)
	  ...
	  while true; do
		...
		case "$choice" in
		  [Yy])
			kj_ssh_read_host_port "请输入远端服务器IP:  " "目標伺服器SSH連接埠 [預設22]:" "22"
...
for entry in data:
    ...
    if isinstance(vec, dict) and vec.get("enabled"):
        vec_str = "就绪" if vec.get("available") else "已啟用(不可用)"
    else:
        vec_str = "未啟用"
    print("向量庫: %s" % vec_str)
